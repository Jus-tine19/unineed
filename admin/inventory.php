                        ...
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <select class="form-select" name="product_id" id="productSelect" required onchange="loadVariants()">
                                    ...
                                    <?php
                                    $opts_q = "SELECT p.product_id, p.product_name, p.stock_quantity as base_stock, p.price,
                                                      COUNT(v.variant_id) as variant_count,
                                                      GROUP_CONCAT(DISTINCT CONCAT(v.variant_id, '::', v.variant_type, '::', v.variant_value, '::', v.stock_quantity, '::', v.price) SEPARATOR '||') as variants
                                                FROM products p
                                                LEFT JOIN product_variants v ON p.product_id = v.product_id
                                                GROUP BY p.product_id
                                    ...
                                    if ($opts_res) {
                                        while ($popt = mysqli_fetch_assoc($opts_res)):
                                            ...
                                            $base_price = isset($popt['price']) ? floatval($popt['price']) : 0;
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" class="form-control" name="quantity" id="quantityInput" required min="1">
                            </div>
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td><strong><?php echo htmlspecialchars($movement['product_name']); ?></strong>
                                            <?php if (!empty($movement['variant_type'])): ?>
                                                <br><small class="text-muted"><?php echo htmlspecialchars($movement['variant_type'] . ': ' . $movement['variant_value']); ?></small>
                                            <?php endif; ?>
                                        </td>
